...
from src.models.internal_task import TaskStatus, TaskType, Priority
...
async def complete_internal_task_api(
    ...
    """
    try:
        production_service = ProductionManagementService(db_session_factory=lambda: db_session, event_bus=event_bus)
        completed_task = await production_service.complete_internal_task(task_id=task_id, actual_end_date=datetime.utcnow())
...
@router.get("/deliverables/{deliverable_id}/tasks/", response_model=List[InternalTaskResponse])
