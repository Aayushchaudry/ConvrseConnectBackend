...
async def trigger_info_gathering_debug(
    ...
    try:
        logger.info(f"🔧 DEBUG: Triggering info gathering for project {request.project_id}")
...
async def submit_client_feedback_debug(
    ...
    try:
        logger.info(f"🔧 DEBUG: Submitting client feedback for review item {request.review_item_id}")
        ...
        feedback = ClientFeedback(
            ...
            feedback_type=FeedbackType.ACCEPT if request.feedback_type == "ACCEPT" else FeedbackType.REJECT,
...
async def test_event_bus_endpoint(
    ...
    try:
        ...
        return {
            "status": "error",
            "message": str(e),
            "event_bus_type": str(type(event_bus)) if 'event_bus' in locals() else "unknown"
        }
...
async def test_deliverable_service_event_bus(
    ...
    try:
        ...
        logger.info(f"🔧 DELIVERABLE DEBUG: Service created with event_bus: {type(deliverable_service.event_bus)}")
