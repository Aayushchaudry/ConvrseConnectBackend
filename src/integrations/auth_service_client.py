...
class AuthServiceClient:
        ...
        for attempt in range(self.config.auth_service_max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise AuthServiceError(f"Auth service error: {response.status_code}")
                    
            except httpx.RequestError as e:
                logger.warning(f"Auth service request failed (attempt {attempt + 1}): {e}")
                if attempt == self.config.auth_service_max_retries - 1:
                    self.circuit_breaker.call_failed()
                    raise AuthServiceError(f"Auth service unreachable after {self.config.auth_service_max_retries} attempts")
    ...
    async def get_user_data(self, user_id: int) -> UserData:
        ...
        try:
            response = await self._make_request(
                "GET",
                f"{self.endpoints.get_user}?user_id={user_id}"
            )
    ...
    async def get_user_businesses(self, user_id: int) -> List[BusinessData]:
        ...
        try:
            response = await self._make_request(
                "GET",
                f"{self.endpoints.get_user_businesses}?user_id={user_id}"
            )
