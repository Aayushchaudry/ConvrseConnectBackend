...
class InitiateDeliverableProductionCommand(BaseCommand):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID):
        super().__init__(command_id=uuid.uuid4(), timestamp=datetime.utcnow(), command_type="InitiateDeliverableProductionCommand")
...
class GenerateReviewItemCommand(BaseCommand):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID, source_internal_task_id: UUID, item_type: str, item_url: str, review_round: int):
        super().__init__(command_id=uuid.uuid4(), timestamp=datetime.utcnow(), command_type="GenerateReviewItemCommand")
        self.project_id = project_id
        self.deliverable_id = deliverable_id
...
class UpdateDeliverableStatusCommand(BaseCommand):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID, new_status: str):
        super().__init__(command_id=uuid.uuid4(), timestamp=datetime.utcnow(), command_type="UpdateDeliverableStatusCommand")
        self.project_id = project_id
        self.deliverable_id = deliverable_id
