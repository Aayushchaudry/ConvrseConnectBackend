...
class UpdateInternalTaskStatusCommand(BaseCommand):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID, task_id: UUID, new_status: str):
        super().__init__(command_id=uuid.uuid4(), timestamp=datetime.utcnow(), command_type="UpdateInternalTaskStatusCommand")
        self.project_id = project_id
        self.deliverable_id = deliverable_id
...
class CreateReworkTaskCommand(BaseCommand):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID, original_task_id: UUID, comment_id: UUID, rework_description: str):
        super().__init__(command_id=uuid.uuid4(), timestamp=datetime.utcnow(), command_type="CreateReworkTaskCommand")
        self.project_id = project_id
        self.deliverable_id = deliverable_id
