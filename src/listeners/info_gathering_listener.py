...
from src.services.info_gathering_service import InformationGatheringService # Import the service to call
from src.commands.project_commands import StartInformationGatheringCommand # Import the command to consume
...
async def start_listening(event_bus: EventBus):
    ...
    """
    logger.info(f"Information Gathering Listener starting for topic: {INFO_GATHERING_COMMAND_TOPIC}")
    ...
    consumer = event_bus.get_consumer(
        ...
        group_id=settings.KAFKA_CONSUMER_GROUP_ID + "-info-gatherer" # A distinct consumer group ID
    )
    ...
    info_gathering_task = asyncio.create_task(
        _listen_loop(consumer, info_gathering_service.handle_start_information_gathering_command)
    )
...
async def _listen_loop(consumer: Any, handler: Callable[[Dict[str, Any]], Any]):
    ...
    try:
        ...
        async for message in consumer:
            logger.info(f"Listener received command from Topic='{message.topic}', Offset={message.offset}")
            try:
                ...
                logger.error(f"Error processing command in info_gathering_listener handler: {e}", exc_info=True)
