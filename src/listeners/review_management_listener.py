...
async def start_listening(event_bus: EventBus):
    ...
    """
    logger.info(f"Review Management Listener starting for topics: {REVIEW_MANAGEMENT_COMMAND_TOPICS}")
    ...
    consumer = event_bus.get_consumer(
        ...
        group_id=settings.KAFKA_CONSUMER_GROUP_ID + "-review-manager" # A distinct consumer group ID
    )
...
async def _listen_loop(consumer: Any, service_instance: ReviewManagementService):
    ...
    try:
        ...
        async for message in consumer:
            logger.info(f"Review Management Listener received command: Topic='{message.topic}', Offset={message.offset}, Type='{message.value.get('command_type')}'")
            try:
                ...
                if command_type == "GenerateReviewItemCommand":
                    command_obj = deliverable_saga_commands.GenerateReviewItemCommand(**command_data)
                    handler_method = service_instance.handle_generate_review_item_command
                ...
                if handler_method and command_obj:
                    ...
                    logger.warning(f"No handler or command class found for command type: {command_type} in ReviewManagementListener. Data: {command_data}")

            except Exception as e:
                logger.error(f"Error processing command in review_management_listener handler for {command_type}: {e}", exc_info=True)
