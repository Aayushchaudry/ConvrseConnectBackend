...
DELIVERABLE_COMMANDS_TOPIC = "project.command.start_deliverable_saga" # From Project Orchestrator
...
async def start_listening(event_bus: EventBus):
    ...
    """
    logger.info(f"Deliverable Events Listener starting for topics: {DELIVERABLE_EVENTS_TOPICS} and {DELIVERABLE_COMMANDS_TOPIC}")
    ...
    try:
        for message in consumer:
            ...
            try:
                ...
                logger.error(f"Error processing message in deliverable_events_listener handler: {e}", exc_info=True)
