...
from src.services.production_management_service import (
...
async def start_listening(event_bus: EventBus):
    ...
    """
    logger.info(
        f"Production Management Listener starting for topics: {PRODUCTION_COMMAND_TOPICS}"
    )
    ...
    consumer = event_bus.get_consumer(
        ...
        + "-production-manager",  # A distinct consumer group ID
    )
    logger.info(
        f"Production Management Listener subscribed to topics: {PRODUCTION_COMMAND_TOPICS}"
    )
    ...
    try:
        ...
        async for message in consumer:
            ...
            if message.topic == "deliverable.command.initiate_modeling":
                ...
                logger.info(
                    f"Created modeling task for deliverable {command.deliverable_id}"
                )
                ...
                logger.info(
                    f"Created rework task for original task {command.original_task_id}"
                )
                ...
                await service_instance.handle_update_internal_task_status_command(
                    command
                )
                logger.info(
                    f"Updated internal task {command.task_id} status to {command.new_status}"
                )
