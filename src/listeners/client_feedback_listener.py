...
from src.orchestrators.deliverable_saga_orchestrator.deliverable_saga_orchestrator import DeliverableSagaOrchestrator # Import the orchestrator to call
from src.services.review_management_service import ReviewManagementService # Import the service to call
from src.events.client_feedback_events import ClientFeedbackSubmittedEvent # Import the event to consume

logger = logging.getLogger(__name__)

# Define the Kafka topic this listener will subscribe to for client feedback events
CLIENT_FEEDBACK_TOPIC = "client.feedback.submitted" # Topic where ClientFeedbackSubmittedEvent is published
...
async def start_listening(event_bus: EventBus):
    ...
    consumer = event_bus.get_consumer(
        ...
        group_id=settings.KAFKA_CONSUMER_GROUP_ID + "-client-feedback" # A distinct consumer group ID
    )

    # Start the async task for the consumer loop
    client_feedback_listener_task = asyncio.create_task(
        _listen_loop(consumer, deliverable_orchestrator, review_service) # Pass both handlers
    )
...
async def _listen_loop(consumer: Any, orchestrator: DeliverableSagaOrchestrator, review_service: ReviewManagementService):
    """
    ...
    try:
        ...
        async for message in consumer:
            logger.info(f"Client Feedback Listener received event: Topic='{message.topic}', Offset={message.offset}, Type='{message.value.get('event_type')}'")
            try:
                ...
                if event_type == "ClientFeedbackSubmittedEvent":
                    ...
                    logger.info(f"Client Feedback Listener: Feedback stored in database for ReviewItem {event.review_item_id}")
                    
                    # 2. DeliverableSagaOrchestrator to process SAGA state transitions
                    await orchestrator.handle_event(event_data)
                    logger.info(f"Client Feedback Listener: SAGA state updated for Deliverable {event.deliverable_id}")
                else:
                    logger.warning(f"Client Feedback Listener: Unexpected event type {event_type}, expected ClientFeedbackSubmittedEvent")
                    
            except Exception as e:
                logger.error(f"Error processing message in client_feedback_listener handler: {e}", exc_info=True)
