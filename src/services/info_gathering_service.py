...
from src.config.database import AsyncSessionLocal # For getting session if needed by handlers
from src.events.event_bus_interface import EventBus

from src.models.project import Project
from src.models.deliverable import Deliverable, DeliverableType # Import DeliverableType for mapping requirements
from src.models.requirement import Requirement, RequirementType, RequirementStatus
from src.commands.project_commands import StartInformationGatheringCommand # Command to consume
from src.events.deliverable_events import DeliverableInfoGatheredEvent, DeliverableInfoGatheringFailedEvent # Events to publish
...
DELIVERABLE_REQUIREMENTS_MAP: Dict[DeliverableType, List[tuple]] = {
    DeliverableType.RENDERED_IMAGES: [
        ...
        ("Material Detail", RequirementType.FILE_UPLOAD, False), # Can be file or text, assuming file for now
    ...
    DeliverableType.VIDEO_WALKTHROUGH: [
        ("Floor Plan Cad File", RequirementType.FILE_UPLOAD, True), # Assuming Interior focus for base
        ("Theme/Mood Board", RequirementType.FILE_UPLOAD, True),
        ("Material Detail", RequirementType.FILE_UPLOAD, False),
        ("Texture Detail", RequirementType.FILE_UPLOAD, False),
        ("Furniture Detail", RequirementType.FILE_UPLOAD, False),
    ...
    DeliverableType.INVENTORY_MODULE: [
        ("Inventory Details", RequirementType.FILE_UPLOAD, True), # Assuming a data file like CSV
    ...
    DeliverableType.LOCATION_MAP: [
        ("Project Location", RequirementType.TEXT_INPUT, True), # Or specific Lat/Long input
    ...
    DeliverableType.INTERPLAYER_SOFTWARE: [
        ...
        ("Scope Confirmation", RequirementType.BOOLEAN_INPUT, True), # Simple confirmation
    ],
}
...
class InformationGatheringService:
    ...
    async def handle_start_information_gathering_command(self, command: StartInformationGatheringCommand):
        """
        ...
        """
        logger.info(f"InformationGatheringService: Received StartInformationGatheringCommand for Project ID: {command.project_id}")

        try:
            async with self.db_session_factory() as session:
                # Fetch the project (optional, but good for context/validation)
                from sqlalchemy import select
                project = await session.execute(select(Project).filter(Project.id == command.project_id))
                project = project.scalar_one_or_none()
                if not project:
                    logger.error(f"InformationGatheringService: Project {command.project_id} not found for command. Skipping.")
                    # Publish a failure event if necessary, but this indicates a larger issue.
                    return

                logger.info(f"InformationGatheringService: Found project {project.name} for ID {command.project_id}")
                ...
                if command.deliverable_ids:
                    logger.info(f"InformationGatheringService: Processing specific deliverable IDs: {command.deliverable_ids}")
                    ...
                    logger.info(f"InformationGatheringService: Processing all deliverables for project {command.project_id}")
                    deliverables_results = await session.execute(
                        select(Deliverable).filter(Deliverable.project_id == command.project_id)
                    )
                    deliverables_to_process = deliverables_results.scalars().all()

                logger.info(f"InformationGatheringService: Found {len(deliverables_to_process)} deliverables to process")

                if not deliverables_to_process:
                    logger.warning(f"InformationGatheringService: No deliverables found for project {command.project_id} to process. Command finished.")
                    return

                for deliverable in deliverables_to_process:
                    logger.info(f"InformationGatheringService: Processing deliverable {deliverable.id} ({deliverable.deliverable_type.value}) for requirements.")
                    ...
                    if not required_items:
                        logger.warning(f"No predefined requirements found for deliverable type: {deliverable.deliverable_type.value}. Skipping requirement creation.")
                    ...
                    try:
                        for req_name, req_type_enum, is_mandatory in required_items:
                            ...
                            if existing_req.scalar_one_or_none():
                                logger.debug(f"Requirement '{req_name}' already exists for deliverable {deliverable.id}. Skipping creation.")
                            ...
                            logger.info(f"Created requirement: '{req_name}' for Deliverable {deliverable.id}")
                        ...
                        await self.event_bus.publish(
                            ...
                            message=DeliverableInfoGatheringFailedEvent(
                                ...
                                error_details={"deliverable_type": deliverable.deliverable_type.value}
                            ).__dict__
                        )
                        await session.rollback() # Rollback changes for this deliverable if an error occurs

                logger.info(f"InformationGatheringService: Finished processing StartInformationGatheringCommand for Project {command.project_id}.")
        except Exception as e:
            logger.error(f"Error handling StartInformationGatheringCommand: {e}", exc_info=True)
