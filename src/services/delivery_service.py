...
from src.events.project_events import DeliverableDeliveredEvent as ProjectDeliverableDeliveredEvent # Alias for clarity if similar name exists
...
class DeliveryService:
    ...
    async def _get_context_entities(self, session: AsyncSession, project_id: UUID, deliverable_id: UUID):
    ...
    async def handle_generate_final_output_command(self, command: GenerateFinalOutputCommand):
        ...
        """
        logger.info(f"DeliveryService: Received GenerateFinalOutputCommand for Deliverable {command.deliverable_id} (Output: {command.output_name})")

        async with self.db_session_factory() as session:
            try:
                project, deliverable = await self._get_context_entities(session, command.project_id, command.deliverable_id)
                ...
                existing_output_result = await session.execute(
                    select(ProjectOutput).filter(
                        ...
                        ProjectOutput.output_name == command.output_name # Assuming unique name per deliverable
                    )
                )
                existing_output = existing_output_result.scalar_one_or_none()
                if existing_output:
                    logger.warning(f"Project output '{command.output_name}' already exists for Deliverable {command.deliverable_id}. Skipping creation. Idempotent.")
                ...
                logger.info(f"DeliveryService: Created new ProjectOutput {new_project_output.id} for Deliverable {command.deliverable_id}.")
                ...
                logger.error(f"DeliveryService Error: Context entity not found for command: {ve}")
                # Publish DeliverableFailedEvent from Project_events to orchestrator
                await self._publish_delivery_failed_event(command.project_id, command.deliverable_id, deliverable.deliverable_type.value if deliverable else "Unknown", f"Context missing: {str(ve)}")
            except Exception as e:
                logger.error(f"Error generating final output for Deliverable {command.deliverable_id}: {e}", exc_info=True)
                await session.rollback()
                # Publish DeliverableFailedEvent (from project_events)
                await self._publish_delivery_failed_event(command.project_id, command.deliverable_id, deliverable.deliverable_type.value if deliverable else "Unknown", f"Failed to generate output: {str(e)}")

    async def _publish_delivery_failed_event(self, project_id: UUID, deliverable_id: UUID, deliverable_name: str, reason: str):
        """Helper to publish DeliverableFailedEvent if delivery fails."""
        from src.events.project_events import DeliverableFailedEvent as ProjectDeliverableFailedEvent # Alias
        await self.event_bus.publish(
