...
class SagaProcessor:
    ...
    async def get_or_create_saga_state(
        ...
        if saga_id:
            ...
            if saga_state:
                logger.info(f"Retrieved existing SAGA state (ID: {saga_id}) for {saga_type.value} project {project_id}.")
                return saga_state
        ...
        query = select(SagaState).filter(
            ...
            SagaState.saga_type == saga_type
        )
        if deliverable_id:
            query = query.filter(SagaState.deliverable_id == deliverable_id)
        else:
            query = query.filter(SagaState.deliverable_id.is_(None)) # Ensure no deliverable_id for project saga
        ...
        if saga_state is None:
            ...
            logger.info(f"New SAGA state created for {saga_type.value} project {project_id}. SAGA ID: {new_saga_state.saga_id}")
            return new_saga_state
        else:
            logger.info(f"Existing SAGA state found for {saga_type.value} project {project_id}. Current state: {saga_state.current_state}")
            return saga_state
    ...
    async def update_saga_state(
        ...
        new_saga_status: Optional[SagaStatus] = None # Optional, for final statuses like COMPLETED, FAILED
    ):
        """
        ...
        if saga_state.last_event_processed_id == str(event_id):
            logger.warning(f"SAGA {saga_state.saga_id} already processed event {event_id}. Skipping update.")
            return
        ...
        logger.info(f"SAGA {saga_state.saga_id} (Type: {saga_state.saga_type.value}) state updated to '{new_state_enum.value}'. Status: {saga_state.status.value}")
    ...
    async def publish_message(self, topic: str, message_payload: Any):
        ...
        logger.info(f"Published message to topic '{topic}': {message_payload.command_type if hasattr(message_payload, 'command_type') else message_payload.event_type}")