...
from src.models.requirement import Requirement # Import Requirement model for ForeignKey
...
class RequirementFile(Base):
    ...
    file_name = Column(String(255), nullable=False) # Original name of the uploaded file
    file_path = Column(Text, nullable=False) # URL to the file in cloud storage (e.g., S3 URL)
    ...
    requirement_ref = relationship("Requirement", backref="requirement_files", lazy="joined")
    ...
    def __repr__(self):
        ...
        return (f"<RequirementFile(id='{self.id}', requirement_id='{self.requirement_id}', "
