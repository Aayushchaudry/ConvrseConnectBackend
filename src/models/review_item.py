...
from src.models.internal_task import InternalTask # Import InternalTask model for ForeignKey
...
class ReviewStatus(enum.Enum):
    ...
    REVISIONS_PENDING = "revisions_pending" # Revisions requested by client, awaiting new version
...
class ReviewItem(Base):
    ...
    source_internal_task_id = Column(UUID(as_uuid=True), ForeignKey('internal_tasks.id'), nullable=False) # The task that generated this output
    
    item_type = Column(Enum(ReviewItemType), nullable=False) # Type of content being reviewed
    item_url = Column(Text, nullable=False) # URL to the asset (e.g., image URL, video URL, prototype link)
    description = Column(Text, nullable=True) # Description/context for this specific review item

    review_status = Column(Enum(ReviewStatus), default=ReviewStatus.PENDING, nullable=False) # Current status from client's review perspective
    
    # For review batches or specific rounds
    sequence_number = Column(Integer, nullable=True) # Order in a list of options (e.g., render option 1, 2)
    review_round = Column(Integer, nullable=True) # Which revision round this item belongs to (e.g., 1st draft, 2nd revision)

    presented_at = Column(DateTime, default=func.now(), nullable=False) # When this item was presented for review
    ...
    deliverable_ref = relationship("Deliverable", backref="review_items_deliverable", lazy="joined")
    source_internal_task_ref = relationship("InternalTask", foreign_keys=[source_internal_task_id], backref="generated_review_items", lazy="joined")
    ...
    def __repr__(self):
        ...
        return (f"<ReviewItem(id='{self.id}', type='{self.item_type.value}', "
