...
import enum
import uuid
from sqlalchemy import Column, String, DateTime, func, Boolean, Text, Enum, ForeignKey
...
from src.models.project import Project # Import Project model for ForeignKey (for convenience/denormalization)
...
class RequirementStatus(enum.Enum):
    ...
    NOT_APPLICABLE = "not_applicable" # This requirement does not apply to this deliverable instance
...
class Requirement(Base):
    ...
    project_id = Column(UUID(as_uuid=True), ForeignKey('projects.id'), nullable=False) # For convenience in queries
    ...
    value = Column(Text, nullable=True) # Use Text for JSON_INPUT if you store stringified JSON
    ...
    def __repr__(self):
        ...
        return (f"<Requirement(id='{self.id}', name='{self.requirement_name}', "
