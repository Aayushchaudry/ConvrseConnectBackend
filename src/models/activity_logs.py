...
class ActivityLog(Base):
    ...
    user_id = Column(Integer, nullable=False, index=True) # Foreign key to users table in auth-service
    business_id = Column(Integer, nullable=False, index=True) # Foreign key to businesses table in auth-service
    
    # Action details
    action = Column(String(100), nullable=False, index=True) # Action performed (e.g., "project_created", "deliverable_updated")
    resource_type = Column(String(50), nullable=False, index=True) # Type of resource (e.g., "project", "deliverable")
    resource_id = Column(String(100), nullable=True, index=True) # ID of the affected resource
    
    # Additional context and details
    details = Column(JSONB, nullable=True) # JSON object with additional context
    ip_address = Column(String(45), nullable=True) # Client IP address
    user_agent = Column(Text, nullable=True) # Client user agent
    
    # Service context
    service_name = Column(String(50), nullable=False, default="convrse-connect-backend") # Source service
    correlation_id = Column(String(36), nullable=True, index=True) # Request correlation ID
