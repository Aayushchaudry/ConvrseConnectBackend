...
class AuthConfig(BaseSettings):
    ...
    auth_service_url: str = Field(
        default="http://localhost:8001", description="Base URL for the auth-service"
    )

    auth_service_token: str = Field(
        default="", description="Service-to-service authentication token"
    )

    auth_service_timeout: int = Field(
        default=30, description="Timeout for auth-service requests in seconds"
    )

    auth_service_max_retries: int = Field(
        default=3, description="Maximum retries for auth-service requests"
    )

    # JWT Configuration
    jwt_secret_key: str = Field(
        default="", description="Secret key for JWT token validation"
    )

    jwt_algorithm: str = Field(
        default="HS256", description="Algorithm for JWT token validation"
    )
    ...
    redis_url: str = Field(
        default="redis://localhost:6379/0", description="Redis URL for token caching"
    )
    ...
    auth_header_name: str = Field(
        default="Authorization", description="Header name for JWT token"
    )

    auth_header_prefix: str = Field(
        default="Bearer", description="Prefix for JWT token in header"
    )
    ...
    audit_log_sensitive_fields: bool = Field(
        default=False, description="Log sensitive fields in audit logs"
    )
        ...
        if env == "test":
            ...
            auth_config = AuthConfig(
                ...
                jwt_secret_key=os.getenv(
                    "JWT_SECRET_KEY",
                    "test-secret-key-for-testing-only-at-least-32-chars",
                ),
                auth_service_timeout=10,
                token_cache_ttl=60,
