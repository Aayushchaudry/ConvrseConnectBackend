...
class DeliverableInfoGatheringFailedEvent(BaseEvent):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID, reason: str, error_details: Optional[Dict[str, Any]] = None):
        super().__init__(event_id=uuid.uuid4(), timestamp=datetime.utcnow(), event_type="DeliverableInfoGatheringFailedEvent")
        self.project_id = project_id
        self.deliverable_id = deliverable_id
        self.reason = reason
...
class RequirementUpdatedEvent(BaseEvent):
    ...
    def __init__(self, project_id: UUID, deliverable_id: UUID, requirement_id: UUID, new_status: str):
        super().__init__(event_id=uuid.uuid4(), timestamp=datetime.utcnow(), event_type="RequirementUpdatedEvent")
        self.project_id = project_id
        self.deliverable_id = deliverable_id
